...
clip_length = config['ucf']['testing']['clip_length']
stride = config['ucf']['testing']['clip_stride']
checkpoint_path = config['testing']['checkpoint_path_ucf']
json_name = config['ucf']['testing']['output_json']
output_path = config['ucf']['testing']['output_path']
...
if __name__ == '__main__':
    video_infos = get_video_info(config['ucf']['testing']['video_info_path'])
    npy_data_path = config['ucf']['testing']['video_data_path']
    net = BDNet(in_channels=config['model']['in_channels'], training=False)
    net.load_state_dict(torch.load(checkpoint_path))
    net.eval().cuda()

    centor_crop = videotransforms.CenterCrop(config['ucf']['testing']['crop_size'])
    ...
    for video_name in tqdm.tqdm(list(video_infos.keys()), ncols=0):
        ...
        realname = str(video_name).split('/')[1].split('.')[0]
        data = np.load(os.path.join(npy_data_path, realname + '.npy'))

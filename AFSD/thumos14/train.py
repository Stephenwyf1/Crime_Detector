...
def save_model(epoch, model, optimizer):
    torch.save(model.module.state_dict(),
               os.path.join(checkpoint_path,
                            'VAD-checkpoint-test:{}-{}.ckpt'.format(config['training']['confidence_margin'], epoch)))
...
def resume_training(resume, model, optimizer):
    ...
    if resume > 0:
        ...
        model_path = os.path.join(checkpoint_path,
                                  'VAD-checkpoint-test:{}-{}.ckpt'.format(config['training']['confidence_margin'],
                                                                        resume))
...
def forward_one_epoch(net, clips, targets, scores=None, training=True, ssl=True):
    ...
    if ssl:
        ...
        loss_l, loss_c, loss_prop_l, loss_prop_c, loss_ct = CPD_Loss(predictions=
                                                                     [output_dict['loc'], output_dict['conf'],
                                                                      output_dict['conf_VAD_score'],
                                                                      output_dict['conf_prop_VAD_score'],
...
def run_one_epoch(epoch, net, optimizer, data_loader, epoch_step_num, training=True):
        ...
        for n_iter, (clips, targets, scores, ssl_clips, ssl_targets, flags) in enumerate(pbar):
            ...
            if training:
                ...
                if n_iter % 20 == 0:
                    ...
                    print('Epoch '+str(epoch)+' : '+str(n_iter // 200)+' , LOSS ='+str(cost.item()))
                cost.backward()
                optimizer.step()
    ...
    if training:
        ...
        np.array(loss_list)
        np.save('output/epoch_loss_Thumos14:{}'.format(epoch),loss_list)
    else:
        prefix = 'Val'
...
if __name__ == '__main__':
    ...
    train_dataset = THUMOS_Dataset(train_data_dict,
                                   train_video_infos,
                                   train_video_annos)
    # train_dataset = THUMOS_Dataset(train_video_infos, train_video_annos)
    from AFSD.common.batch_sampler import BatchSampler

    _batch_sampler = BatchSampler(train_dataset, batch_size)

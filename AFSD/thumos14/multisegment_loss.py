...
class MultiSegmentLoss(nn.Module):
    ...
    def forward(self, predictions, targets, pre_locs=None):
        ...
        """
        loc_data, conf_data, conf_VAD_score, conf_prop_VAD_score, \
        ...
        with torch.no_grad():
            for idx in range(num_batch):
                ...
                label.append(1. if targets[idx][:, -1].cpu().numpy()[0] <= 9 else 0.)
        ...
        # -----------------------------------------------#
        # improved loss function which turn the mutil-prediction result to the two-fold result
        # softmax focal loss
        y_true = label

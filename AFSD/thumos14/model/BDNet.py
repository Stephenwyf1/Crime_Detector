...
from AFSD.thumos14.model.i3d_backbone import InceptionI3d
from AFSD.common.config import config
from AFSD.thumos14.model.layers import Unit1D, Unit3D
...
class CoarsePyramid(nn.Module):
    ...
    def forward(self, feat_dict, ssl=False):
        pyramid_feats = [] #存放三层FPN的每一层抓取的特征
        locs = []
        confs = []
        centers = []
        prop_locs = []
        ...
        frame_level_feat = F.interpolate(frame_level_feat, [self.frame_num, 1]).squeeze(-1) #上采样
        frame_level_feat = self.deconv(frame_level_feat)
        trip.append(frame_level_feat.clone())
        start_feat = frame_level_feat[:, :256] # 存放开始帧的特征信息
        end_feat = frame_level_feat[:, 256:] # 存放结束帧的特征信息
        start = start_feat.permute(0, 2, 1).contiguous()
        end = end_feat.permute(0, 2, 1).contiguous()

        for i, feat in enumerate(pyramid_feats): # 对FPN每一个阶段的特征分别进行回归和分类特征处理
...
class BDNet(nn.Module):
    def __init__(self, in_channels=3, backbone_model=None, training=True):
        ...
        self.FC = nn.Sequential(nn.Linear(in_features=252,out_features=32),
                                nn.ReLU(),
    ...
    def forward(self, x, proposals=None, ssl=False):
        ...
        feat_dict = self.backbone(x) # here is the feature
        if ssl:
            ...
            conf_ = conf.view(conf.size(0),-1)
            prop_conf_ = prop_conf.view(prop_conf.size(0),-1)
            score = self.FC(conf_)
            score_prop = self.FC(prop_conf_)
            return {
                'loc': loc,
                'conf': conf,
                'conf_VAD_score': score,
                'conf_prop_VAD_score': score_prop,
                'priors': priors,
                'prop_loc': prop_loc,
                'prop_conf': prop_conf,
                'center': center,
...
def test_inference(clip_frames=256):
    model = BDNet(training=False)
    model.eval()
    model.cuda()
    x = torch.randn([1, 3, clip_frames, 96, 96]).cuda()
    state = {'model':model.state_dict()}
    torch.onnx.export(model,x,'BDNet.proto')
    # torch.save(state,'BDNet.pt')
